...
_HERO_WIN_RE = re.compile(r"^Win as (.+?)$")
...
def set_dota_rules(world:DOTA2World)-> None:
    ...
    primordial_gate = world._effective_primordial_fragments_to_unlock_final()
    ...
    for loc in multiworld.get_locations(player):
        ...
        if final_hero_name and hero == final_hero_name:
            ...
            set_rule(
                loc,
                lambda state, p=player, req=primordial_gate, macguffin=FILLER_ITEM_NAME: state.has(macguffin, p) >= req,
            )
        else:
...
# def set_all_rules(world: DOTA2World) -> None:
...
# def set_all_entrance_rules(world: DOTA2World) -> None:
...
# def set_all_location_rules(world: DOTA2World) -> None:
...
# def set_completion_condition(world: DOTA2World) -> None:
